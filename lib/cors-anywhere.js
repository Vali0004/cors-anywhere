...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    allowedTargets: [],             // If non-empty, any requests not in this list will be blocked.
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.allowedTargets.length > 0) {
      const hostname = location.hostname.toLowerCase();
      if (!corsAnywhere.allowedTargets.some(domain => {
